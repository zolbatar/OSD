...
void Tokeniser::PrintTokens(std::list<Token> *tokens, int depth, bool stage1)
{
...
void Tokeniser::PrintToken(Token *token, int depth, std::list<std::string> *output)
{
    ...
    // Header
    char l[256];
    sprintf(l, "%s%3d [%3d]: ", spacing.c_str(), token->char_number, (int)token->type);
    ...
    case TokenType::ASSIGNMENT:
        ...
        else if (token->vtype.IsType())
        {
            sprintf(m, "Type assignment '%s' (type %s), index %" PRId64 "", token->name.c_str(),
                    token->vtype.GetType()->name.c_str(), token->index);
        }
        break;
