...
static const struct
{
...
#ifdef CAPSTONE_HAS_MIPS
    {
        Mips_global_init,
        Mips_option,
        ~(CS_MODE_LITTLE_ENDIAN | CS_MODE_32 | CS_MODE_64 | CS_MODE_MICRO | CS_MODE_MIPS32R6 | CS_MODE_BIG_ENDIAN |
...
#ifdef CAPSTONE_HAS_POWERPC
    {
        PPC_global_init,
        PPC_option,
        ~(CS_MODE_LITTLE_ENDIAN | CS_MODE_32 | CS_MODE_64 | CS_MODE_BIG_ENDIAN | CS_MODE_QPX),
    },
#else
    {NULL, NULL, 0},
#endif
...
#ifdef CAPSTONE_HAS_M68K
    {
        M68K_global_init,
        M68K_option,
        ~(CS_MODE_BIG_ENDIAN | CS_MODE_M68K_000 | CS_MODE_M68K_010 | CS_MODE_M68K_020 | CS_MODE_M68K_030 |
...
#ifdef CAPSTONE_HAS_M680X
    {
        M680X_global_init,
        M680X_option,
        ~(CS_MODE_M680X_6301 | CS_MODE_M680X_6309 | CS_MODE_M680X_6800 | CS_MODE_M680X_6801 | CS_MODE_M680X_6805 |
          CS_MODE_M680X_6808 | CS_MODE_M680X_6809 | CS_MODE_M680X_6811 | CS_MODE_M680X_CPU12 | CS_MODE_M680X_HCS08),
    },
#else
    {NULL, NULL, 0},
#endif
...
#ifdef CAPSTONE_HAS_MOS65XX
    {
        MOS65XX_global_init,
        MOS65XX_option,
        ~(CS_MODE_LITTLE_ENDIAN | CS_MODE_MOS65XX_6502 | CS_MODE_MOS65XX_65C02 | CS_MODE_MOS65XX_W65C02 |
...
#ifdef CAPSTONE_HAS_BPF
    {
        BPF_global_init,
        BPF_option,
        ~(CS_MODE_LITTLE_ENDIAN | CS_MODE_BPF_CLASSIC | CS_MODE_BPF_EXTENDED | CS_MODE_BIG_ENDIAN),
    },
#else
    {NULL, NULL, 0},
#endif
...
bool CAPSTONE_API cs_support(int query)
{
    if (query == CS_ARCH_ALL)
        return all_arch ==
               ((1 << CS_ARCH_ARM) | (1 << CS_ARCH_ARM64) | (1 << CS_ARCH_MIPS) | (1 << CS_ARCH_X86) |
                (1 << CS_ARCH_PPC) | (1 << CS_ARCH_SPARC) | (1 << CS_ARCH_SYSZ) | (1 << CS_ARCH_XCORE) |
                (1 << CS_ARCH_M68K) | (1 << CS_ARCH_TMS320C64X) | (1 << CS_ARCH_M680X) | (1 << CS_ARCH_EVM) |
                (1 << CS_ARCH_RISCV) | (1 << CS_ARCH_MOS65XX) | (1 << CS_ARCH_WASM) | (1 << CS_ARCH_BPF));
...
static void fill_insn(struct cs_struct *handle, cs_insn *insn, char *buffer, MCInst *mci, PostPrinter_t postprinter,
                      const uint8_t *code)
{
#ifndef CAPSTONE_DIET
    char *sp, *mnem;
...
size_t CAPSTONE_API cs_disasm(csh ud, const uint8_t *buffer, size_t size, uint64_t offset, size_t count, cs_insn **insn)
{
    ...
    while (size > 0)
    {
        ...
        else
        {
            ...
            {
                skipdata_bytes = handle->skipdata_setup.callback(buffer_org, size_org, (size_t)(offset - offset_org),
...
#else
            strncpy(insn_cache->mnemonic, handle->skipdata_setup.mnemonic, sizeof(insn_cache->mnemonic) - 1);
...
bool CAPSTONE_API cs_disasm_iter(csh ud, const uint8_t **code, size_t *size, uint64_t *address, cs_insn *insn)
{
    ...
    { // encounter a broken instruction
        ...
        if (handle->skipdata_setup.callback)
        {
            skipdata_bytes = handle->skipdata_setup.callback(*code, *size, 0, handle->skipdata_setup.user_data);
            if (skipdata_bytes > *size)
                // remaining data is not enough
                return false;
        ...
        strncpy(insn->mnemonic, handle->skipdata_setup.mnemonic, sizeof(insn->mnemonic) - 1);
...
CAPSTONE_EXPORT
int CAPSTONE_API cs_op_index(csh ud, const cs_insn *insn, unsigned int op_type, unsigned int post)
{
    struct cs_struct *handle;
    unsigned int count = 0, i;
    if (!ud)
...
CAPSTONE_EXPORT
cs_err CAPSTONE_API cs_regs_access(csh ud, const cs_insn *insn, cs_regs regs_read, uint8_t *regs_read_count,
