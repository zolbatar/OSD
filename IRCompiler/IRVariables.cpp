	...
	if (token->vtype.IsInteger())
		...
		for (auto& t : token->vtype.GetType()->fields) {
			if (t.second.type==ValueType::String) {
				Init_AddIRWithIndexAndInteger(IROpcodes::VariableLocalCreateTypeString, token->index, t.second.position);
			}
		}
	}
	else
...
void IRCompiler::CompileTokenGlobalVariable(Token* token)
{
	...
	if (token->vtype.IsInteger())
		...
		for (auto& t : token->vtype.GetType()->fields) {
			if (t.second.type==ValueType::String) {
				Init_AddIRWithIndexAndInteger(IROpcodes::VariableGlobalCreateTypeString, token->index, t.second.position);
			}
		}
