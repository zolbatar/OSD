	...
	if (optimise) {
		...
		for (auto node = _jitc->head; node; node = node->next) {
			...
			if (node->code==jit_code_addi && node->u.w==node->v.w) {
				...
				if (next->code==jit_code_prepare &&
						nextnext->code==jit_code_subi &&
						nextnext->u.w==nextnext->v.w && node->u.w==nextnext->v.w &&
						nextnext->w.w==node->w.w) {
...
void NativeCompiler::IRToNativeSection(std::list<IRInstruction>* ir)
{
	for (auto& op: *ir) {
		switch (op.type) {
			case IROpcodes::End:
