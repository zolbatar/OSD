...
#ifdef CLION
            std::unique_lock<std::mutex> guard(message_lock, std::defer_lock);
            if (guard.try_lock()) {
                local_message_queue.swap(message_queue);
            }
#else
            local_message_queue.swap(message_queue);
#endif
            while (!local_message_queue.empty()) {
                auto message = local_message_queue.front();
