...
#if GET_JIT_SIZE
void _jit_size_prepare(jit_state_t *_jit)
{
    _jitc->cptr = _jit->code.ptr;
    _jitc->size = _jit->pc.w;
}

void _jit_size_collect(jit_state_t *_jit, jit_node_t *node)
{
    jit_word_t length;
...
#else
jit_word_t _jit_get_size(jit_state_t *_jit)
{
    jit_word_t size;
    jit_node_t *node;
...
jit_word_t jit_get_max_instr(void)
{
    return (JIT_INSTR_MAX >= 144 ? JIT_INSTR_MAX : 144);
}

void jit_finish_size(void)
{
#if GET_JIT_SIZE
    FILE *fp;
    jit_word_t offset;
