...
static void _jit_dataset(jit_state_t *_jit);
...
static void _jit_follow(jit_state_t *_jit, jit_block_t *block, jit_bool_t *todo);
...
static jit_bool_t _simplify_movr(jit_state_t *_jit, jit_node_t *prev, jit_node_t *node, jit_int32_t kind,
                                 jit_int32_t size);

#define simplify_movi(p, n, k, s) _simplify_movi(_jit, p, n, k, s)
static jit_bool_t _simplify_movi(jit_state_t *_jit, jit_node_t *prev, jit_node_t *node, jit_int32_t kind,
                                 jit_int32_t size);
 ...
 * Implementation
 */
void init_jit(const char *progname)
{
    jit_get_cpu();
...
jit_int32_t _jit_get_reg(jit_state_t *_jit, jit_int32_t regspec)
{
    ...
    if (_jitc->emit)
    {
        ...
        {
            if ((jit_class(_rvs[regno].spec) & spec) == spec && !jit_regset_tstbit(&_jitc->regarg, regno) &&
        ...
        for (regno = 0; regno < _jitc->reglen; regno++)
        {
            if ((jit_class(_rvs[regno].spec) & spec) == spec && !jit_regset_tstbit(&_jitc->regsav, regno) &&
                !jit_regset_tstbit(&_jitc->regarg, regno) && !(regspec & jit_class_nospill))
            {
            spill:
                ...
                if (spec & jit_class_gpr)
                {
                    ...
                    {
                        _jitc->function->regoff[regno] = jit_allocai(sizeof(jit_word_t));
                        _jitc->again = 1;
                    }
#if DEBUG
                ...
                else
                {
                    if (!_jitc->function->regoff[regno])
                    {
                        _jitc->function->regoff[regno] = jit_allocai(sizeof(jit_float64_t));
                        _jitc->again = 1;
                    }
#if DEBUG
                    /* emit_stxi must not need temporary registers */
...
void _jit_unget_reg(jit_state_t *_jit, jit_int32_t regno)
{
    ...
    assert((regno == jit_carry /*&& _NOREG != jit_carry*/) || jit_regset_tstbit(&_jitc->regarg, regno) != 0);
#else
...
jit_bool_t _jit_callee_save_p(jit_state_t *_jit, jit_int32_t regno)
{
    assert(regno >= 0 && regno < JIT_NOREG);
    return (!!(_rvs[regno].spec & jit_class_sav));
}

extern jit_bool_t _jit_pointer_p(jit_state_t *_jit, jit_pointer_t address)
{
    return ((jit_uint8_t *)address >= _jit->code.ptr && (jit_word_t)address < _jit->pc.w);
}

#if __ia64__
void jit_regset_com(jit_regset_t *u, jit_regset_t *v)
{
    u->rl = ~v->rl;
    u->rh = ~v->rh;
    u->fl = ~v->fl;
...
void jit_regset_and(jit_regset_t *u, jit_regset_t *v, jit_regset_t *w)
{
    u->rl = v->rl & w->rl;
    u->rh = v->rh & w->rh;
    u->fl = v->fl & w->fl;
...
void jit_regset_ior(jit_regset_t *u, jit_regset_t *v, jit_regset_t *w)
{
    u->rl = v->rl | w->rl;
    u->rh = v->rh | w->rh;
    u->fl = v->fl | w->fl;
...
void jit_regset_xor(jit_regset_t *u, jit_regset_t *v, jit_regset_t *w)
{
    u->rl = v->rl ^ w->rl;
    u->rh = v->rh ^ w->rh;
    u->fl = v->fl ^ w->fl;
...
void jit_regset_set(jit_regset_t *u, jit_regset_t *v)
{
    u->rl = v->rl;
    u->rh = v->rh;
    u->fl = v->fl;
...
void jit_regset_set_mask(jit_regset_t *u, jit_int32_t v)
{
    jit_bool_t w = !!(v & (v - 1));
...
jit_bool_t jit_regset_cmp_ui(jit_regset_t *u, jit_word_t v)
{
    return !((u->rl == v && u->rh == 0 && u->fl == 0 && u->fh == 0));
}

void jit_regset_set_ui(jit_regset_t *u, jit_word_t v)
{
    u->rl = v;
    u->rh = u->fl = u->fh = 0;
}

jit_bool_t jit_regset_set_p(jit_regset_t *u)
{
    return (u->rl || u->rh || u->fl || u->fh);
}

void jit_regset_clrbit(jit_regset_t *set, jit_int32_t bit)
{
...
void jit_regset_setbit(jit_regset_t *set, jit_int32_t bit)
{
    assert(bit >= 0 && bit <= 255);
    if (bit < 64)
        set->rl |= 1LL << bit;
...
jit_bool_t jit_regset_tstbit(jit_regset_t *set, jit_int32_t bit)
{
    assert(bit >= 0 && bit <= 255);
    if (bit < 64)
...
static jit_word_t hash_data(const void *data, jit_word_t length)
{
...
jit_node_t *_jit_data(jit_state_t *_jit, const void *data, jit_word_t length, jit_int32_t align)
{
    jit_word_t key;
    jit_node_t *node;
    ...
    if (((_jitc->data.offset + 7) & -8) + length > _jit->data.length)
    {
        ...
        if (_jitc->data.ptr == NULL)
            ...
            jit_realloc((jit_pointer_t *)&_jitc->data.ptr, _jit->data.length, size);
        _jit->data.length = size;
    }
    if (_jitc->data.table == NULL)
        jit_alloc((jit_pointer_t *)&_jitc->data.table, (_jitc->data.size = 16) * sizeof(jit_node_t *));

    key = hash_data(data, length) & (_jitc->data.size - 1);
    node = _jitc->data.table[key];
    for (; node; node = node->next)
    {
        if (node->v.w == length && memcmp(_jitc->data.ptr + node->u.w, data, length) == 0)
            break;
    }
    ...
    if (!node)
    {
        ...
        if (_jitc->data.count > (_jitc->data.size >> 1) + (_jitc->data.size >> 2) &&
            (_jitc->data.size << 1) > _jitc->data.size)
        {
            ...
            for (i = 0; i < _jitc->data.size; i++)
            {
                ...
                {
                    next = temp->next;
                    key = hash_data(_jitc->data.ptr + temp->u.w, temp->v.w) & ((_jitc->data.size << 1) - 1);
                    temp->next = hash[key];
                    hash[key] = temp;
                }
            }
...
static void _new_pool(jit_state_t *_jit)
{
    ...
    if (_jitc->pool.offset >= _jitc->pool.length)
    {
        ...
        jit_realloc((jit_pointer_t *)&_jitc->pool.ptr, _jitc->pool.length * sizeof(jit_node_t *),
...
static jit_node_t *_new_node(jit_state_t *_jit, jit_code_t code)
{
    jit_node_t *node;
...
static inline jit_node_t *_link_node(jit_state_t *_jit, jit_node_t *node)
{
    if (_jitc->tail)
...
static inline void _free_node(jit_state_t *_jit, jit_node_t *node)
{
    memset(node, 0, sizeof(jit_node_t));
    node->next = _jitc->list;
    _jitc->list = node;
...
jit_state_t *jit_new_state(void)
{
    ...
    jit_alloc((jit_pointer_t *)&_jitc->spill, _jitc->reglen * sizeof(jit_node_t *));
    jit_alloc((jit_pointer_t *)&_jitc->gen, _jitc->reglen * sizeof(jit_int32_t));
    jit_alloc((jit_pointer_t *)&_jitc->values, _jitc->reglen * sizeof(jit_value_t));
    ...
    jit_alloc((jit_pointer_t *)&_jitc->data_info.ptr, (_jitc->data_info.length = 1024) * sizeof(jit_data_info_t));
#endif
...
void _jit_really_clear_state(jit_state_t *_jit)
{
    ...
    _jitc->note.head = _jitc->note.tail = _jitc->note.name = _jitc->note.note = NULL;
...
void _jit_synth_inc(jit_state_t *_jit)
{
    assert(_jitc->synth < 8);
    ++_jitc->synth;
}

jit_node_t *_jit_new_node(jit_state_t *_jit, jit_code_t code)
{
    assert(!_jitc->realize);
    return (link_node(new_node(code)));
}

jit_node_t *_jit_new_node_no_link(jit_state_t *_jit, jit_code_t code)
{
    assert(!_jitc->realize);
...
void _jit_synth_dec(jit_state_t *_jit)
{
    assert(_jitc->synth > 0);
    --_jitc->synth;
}

jit_node_t *_jit_new_node_w(jit_state_t *_jit, jit_code_t code, jit_word_t u)
{
...
jit_node_t *_jit_new_node_f(jit_state_t *_jit, jit_code_t code, jit_float32_t u)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.f = u;
...
jit_node_t *_jit_new_node_d(jit_state_t *_jit, jit_code_t code, jit_float64_t u)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.d = u;
...
jit_node_t *_jit_new_node_p(jit_state_t *_jit, jit_code_t code, jit_pointer_t u)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.p = u;
...
jit_node_t *_jit_new_node_ww(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_word_t v)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.w = u;
...
jit_node_t *_jit_new_node_wp(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_pointer_t v)
{
    return (jit_new_node_ww(code, u, (jit_word_t)v));
}

jit_node_t *_jit_new_node_fp(jit_state_t *_jit, jit_code_t code, jit_float32_t u, jit_pointer_t v)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.f = u;
...
jit_node_t *_jit_new_node_dp(jit_state_t *_jit, jit_code_t code, jit_float64_t u, jit_pointer_t v)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.d = u;
...
jit_node_t *_jit_new_node_pw(jit_state_t *_jit, jit_code_t code, jit_pointer_t u, jit_word_t v)
{
    return (jit_new_node_ww(code, (jit_word_t)u, v));
}

jit_node_t *_jit_new_node_wf(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_float32_t v)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.w = u;
...
jit_node_t *_jit_new_node_wd(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_float64_t v)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.w = u;
...
jit_node_t *_jit_new_node_www(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_word_t v, jit_word_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.w = u;
...
jit_node_t *_jit_new_node_qww(jit_state_t *_jit, jit_code_t code, jit_int32_t l, jit_int32_t h, jit_word_t v,
                              jit_word_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
...
jit_node_t *_jit_new_node_wwf(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_word_t v, jit_float32_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.w = u;
...
jit_node_t *_jit_new_node_wwd(jit_state_t *_jit, jit_code_t code, jit_word_t u, jit_word_t v, jit_float64_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.w = u;
...
jit_node_t *_jit_new_node_pww(jit_state_t *_jit, jit_code_t code, jit_pointer_t u, jit_word_t v, jit_word_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.p = u;
...
jit_node_t *_jit_new_node_pwf(jit_state_t *_jit, jit_code_t code, jit_pointer_t u, jit_word_t v, jit_float32_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.p = u;
...
jit_node_t *_jit_new_node_pwd(jit_state_t *_jit, jit_code_t code, jit_pointer_t u, jit_word_t v, jit_float64_t w)
{
    jit_node_t *node = new_node(code);
    assert(!_jitc->realize);
    node->u.p = u;
...
jit_node_t *_jit_label(jit_state_t *_jit)
{
    jit_node_t *node;
...
jit_node_t *_jit_forward(jit_state_t *_jit)
{
    return (jit_new_node_no_link(jit_code_label));
}

jit_node_t *_jit_indirect(jit_state_t *_jit)
{
    jit_node_t *node;
...
void _jit_link(jit_state_t *_jit, jit_node_t *node)
{
    ...
    assert((node->code == jit_code_label || node->code == jit_code_prolog || node->code == jit_code_epilog) &&
           !node->next);
    jit_link_node(node);
    if (_jitc->blocks.offset >= _jitc->blocks.length)
    {
        ...
        jit_realloc((jit_pointer_t *)&_jitc->blocks.ptr, _jitc->blocks.length * sizeof(jit_block_t),
...
jit_bool_t _jit_forward_p(jit_state_t *_jit, jit_node_t *node)
{
    return (node->code == jit_code_label && !node->next && node != _jitc->tail);
}

jit_bool_t _jit_indirect_p(jit_state_t *_jit, jit_node_t *node)
{
    return (node->code == jit_code_label && !!(node->flag & jit_flag_use));
}

jit_bool_t _jit_target_p(jit_state_t *_jit, jit_node_t *node)
{
...
void _jit_prepare(jit_state_t *_jit)
{
    ...
    _jitc->function->call.argi = _jitc->function->call.argf = _jitc->function->call.size = 0;
...
jit_int32_t _jit_classify(jit_state_t *_jit, jit_code_t code)
{
    ...
    case jit_code_jmpr:
        ...
        break;
    case jit_code_retval_c:
    case jit_code_retval_uc:
    case jit_code_retval_s:
    case jit_code_retval_us:
    case jit_code_retval_i:
    case jit_code_retval_ui:
    ...
    case jit_code_movi_d_ww:
        mask = jit_cc_a0_reg | jit_cc_a0_chg | jit_cc_a1_reg | jit_cc_a1_chg | jit_cc_a2_dbl;
        break;
    case jit_code_negr:
    case jit_code_comr:
    case jit_code_movr:
    ...
    case jit_code_htonr_us:
    case jit_code_htonr_ui:
    case jit_code_htonr_ul:
    case jit_code_ldr_c:
    ...
    case jit_code_movr_d_ww:
        mask = jit_cc_a0_reg | jit_cc_a0_chg | jit_cc_a1_reg | jit_cc_a1_chg | jit_cc_a2_reg;
        break;
    case jit_code_addi:
    case jit_code_addxi:
    case jit_code_addci:
    ...
    case jit_code_qdivi_u:
        mask = jit_cc_a0_reg | jit_cc_a0_rlh | jit_cc_a0_chg | jit_cc_a1_reg | jit_cc_a2_int;
        break;
    ...
    case jit_code_qdivr_u:
        mask = jit_cc_a0_reg | jit_cc_a0_rlh | jit_cc_a0_chg | jit_cc_a1_reg | jit_cc_a2_reg;
        break;
    case jit_code_sti_c:
    case jit_code_sti_s:
    case jit_code_sti_i:
    ...
    for (node = _jitc->head; node; node = node->next)
    {
        ...
        default:
            ...
            if (_jitc->function)
            {
                ...
                if ((mask & (jit_cc_a2_reg | jit_cc_a2_chg)) == (jit_cc_a2_reg | jit_cc_a2_chg))
                    jit_regset_setbit(&_jitc->function->regset, jit_regno(node->w.w));
            }
            break;
        }
    }
...
void _jit_realize(jit_state_t *_jit)
{
    assert(!_jitc->realize);
    if (_jitc->function)
        jit_epilog();
...
void _jit_dataset(jit_state_t *_jit)
{
    jit_uint8_t *ptr;
    jit_node_t *node;
    jit_word_t offset;
...
jit_pointer_t _jit_get_code(jit_state_t *_jit, jit_word_t *length)
{
    assert(_jitc->realize);
    if (length)
    {
...
void _jit_set_code(jit_state_t *_jit, jit_pointer_t ptr, jit_word_t length)
{
    assert(_jitc->realize);
    _jit->code.ptr = ptr;
...
jit_pointer_t _jit_get_data(jit_state_t *_jit, jit_word_t *data_size, jit_word_t *note_size)
{
    assert(_jitc->realize);
    if (data_size)
...
void _jit_set_data(jit_state_t *_jit, jit_pointer_t ptr, jit_word_t length, jit_word_t flags)
{
    assert(_jitc->realize);
...
jit_pointer_t _jit_emit(jit_state_t *_jit)
{
    ...
    _jitc->code.end = _jit->code.ptr + _jit->code.length - jit_get_max_instr();
    ...
    for (;;)
    {
        if ((code = emit_code()) == NULL)
        {
            ...
            for (node = _jitc->head; node; node = node->next)
            {
                if (node->link && (node->code == jit_code_label || node->code == jit_code_epilog))
                    node->flag &= ~jit_flag_patch;
            }
            if (_jit->user_code)
                goto fail;
...
void _jit_frame(jit_state_t *_jit, jit_int32_t frame)
{
    jit_trampoline(frame, 1);
}

void _jit_tramp(jit_state_t *_jit, jit_int32_t frame)
{
    jit_trampoline(frame, 0);
}

void _jit_trampoline(jit_state_t *_jit, jit_int32_t frame, jit_bool_t prolog)
{
    jit_int32_t regno;
...
#define regmask block->regmask
    ...
    for (node = block->label->next; node; node = node->next)
    {
        ...
        default:
            ...
            value = jit_classify(node->code);
            if ((value & jit_cc_a2_reg) && !(node->w.w & jit_regno_patch) && jit_regset_tstbit(&regmask, node->w.w))
            {
            ...
            if ((value & jit_cc_a1_reg) && !(node->v.w & jit_regno_patch) && jit_regset_tstbit(&regmask, node->v.w))
            {
            ...
            if (value & jit_cc_a0_reg)
            {
                ...
                if (value & jit_cc_a0_rlh)
                {
                    if (!(node->u.q.l & jit_regno_patch) && jit_regset_tstbit(&regmask, node->u.q.l))
                    {
                        jit_regset_clrbit(&regmask, node->u.q.l);
                        if (live)
                            jit_regset_setbit(&reglive, node->u.q.l);
                    }
                    if (!(node->u.q.h & jit_regno_patch) && jit_regset_tstbit(&regmask, node->u.q.h))
                    {
                ...
                {
                    if (!(node->u.w & jit_regno_patch) && jit_regset_tstbit(&regmask, node->u.w))
                    {
            ...
            if (value & jit_cc_a0_jmp)
            {
                ...
                else
                {
                    ...
                    for (regno = 0; regno < _jitc->reglen; regno++)
                    {
                        ...
                        if (jit_regset_tstbit(&regmask, regno) && (spec & (jit_class_gpr | jit_class_fpr)) &&
                            !(spec & jit_class_sav))
        ...
        default:
            ...
            if (value & jit_cc_a0_jmp)
            {
                ...
                else
                {
                    ...
                    {
                        spec = jit_class(_rvs[regno].spec);
                        if (jit_regset_tstbit(mask, regno) && (spec & (jit_class_gpr | jit_class_fpr)) &&
                            !(spec & jit_class_sav))
...
static void _thread_jumps(jit_state_t *_jit)
{
    ...
    for (prev = node = _jitc->head; node;)
    {
        ...
        default:
            ...
            {
                if (reverse_jump(prev, node) || shortcut_jump(prev, node))
                    continue;
            }
            break;
        }
...
static jit_bool_t _reverse_jump(jit_state_t *_jit, jit_node_t *prev, jit_node_t *node)
{
    ...
    if (local_next->code != jit_code_jmpi || !(local_next->flag & jit_flag_node))
        return (0);
    /* <cond_jump L0> =><jump L1> <label L0> */

    local_jump = local_next->u.n;
    for (local_prev = local_next, local_next = local_next->next; local_next;
    ...
    for (iter = prev->next; iter; prev = iter, iter = iter->next)
    {
        ...
        default:
            ...
            if ((spec & (jit_cc_a0_reg | jit_cc_a0_chg)) == (jit_cc_a0_reg | jit_cc_a0_chg))
            {
                ...
                {
                    if (regno == jit_regno(iter->u.q.l) || regno == jit_regno(iter->u.q.h))
                        return;
                }
                else
                {
            ...
            if ((spec & (jit_cc_a2_reg | jit_cc_a2_chg)) == (jit_cc_a2_reg | jit_cc_a2_chg))
            {
                                 ...
                                 jit_int32_t size)
{
    ...
    if ((value->kind == jit_kind_register && jit_regno(value->base.q.l) == right &&
                                 ...
                                 jit_int32_t size)
{
    ...
    if (value->kind == kind)
    {
        ...
        {
            if (_jitc->values[offset].kind == kind && memcmp(&node->v.w, &_jitc->values[offset].base.w, size) == 0 &&
...
static jit_bool_t _simplify_ldxi(jit_state_t *_jit, jit_node_t *prev, jit_node_t *node)
{
    ...
    right = jit_regno(node->v.w);
    value = _jitc->values + regno;
    if (regno != right && value->kind == jit_kind_code && value->code == node->code && value->base.q.l == right &&
        value->base.q.h == _jitc->gen[right] && node->w.w == value->disp.w)
    {
        del_node(prev, node);
        return (1);
    }
...
static jit_bool_t _simplify_stxi(jit_state_t *_jit, jit_node_t *prev, jit_node_t *node)
{
    ...
    if (regno != right && value->kind == jit_kind_code && value->code == node->code && value->base.q.l == right &&
        value->base.q.h == _jitc->gen[right] && node->u.w == value->disp.w)
    {
        del_node(prev, node);
        return (1);
    }
...
static void _simplify(jit_state_t *_jit)
{
    ...
    for (prev = NULL, node = _jitc->head; node; prev = node, node = next)
    {
        ...
        case jit_code_load:
            regno = jit_regno(node->u.w);
            if (register_change_p(node->link->next, node, regno) != jit_reg_change)
            {
        ...
        case jit_code_movr:
            regno = jit_regno(node->u.w);
            if (simplify_movr(prev, node, jit_kind_word, sizeof(jit_word_t)))
                simplify_spill(node = prev, regno);
            break;
        case jit_code_movi:
            regno = jit_regno(node->u.w);
            if (simplify_movi(prev, node, jit_kind_word, sizeof(jit_word_t)))
                simplify_spill(node = prev, regno);
            break;
        case jit_code_movr_f:
            regno = jit_regno(node->u.w);
            if (simplify_movr(prev, node, jit_kind_float32, sizeof(jit_float32_t)))
                simplify_spill(node = prev, regno);
            break;
        case jit_code_movi_f:
            regno = jit_regno(node->u.w);
            if (simplify_movi(prev, node, jit_kind_float32, sizeof(jit_float32_t)))
                simplify_spill(node = prev, regno);
            break;
        case jit_code_movr_d:
            regno = jit_regno(node->u.w);
            if (simplify_movr(prev, node, jit_kind_float64, sizeof(jit_float64_t)))
                simplify_spill(node = prev, regno);
            break;
        case jit_code_movi_d:
            regno = jit_regno(node->u.w);
            if (simplify_movi(prev, node, jit_kind_float64, sizeof(jit_float64_t)))
                simplify_spill(node = prev, regno);
            break;
        case jit_code_ldxi_c:
        case jit_code_ldxi_uc:
        ...
        default:
            ...
            if (value & jit_cc_a0_jmp)
                return (jit_reg_change);
                     ...
                     (((value & jit_cc_a0_rlh) && (node->u.q.l == regno || node->u.q.h == regno)) ||
                      (!(value & jit_cc_a0_rlh) && node->u.w == regno)))
                return (jit_reg_change);
            else if ((value & jit_cc_a1_reg) && node->v.w == regno && (value & jit_cc_a1_chg))
                return (jit_reg_change);
            else if ((value & jit_cc_a2_reg) && node->w.w == regno && (value & jit_cc_a2_chg))
                return (jit_reg_change);
        }
    }
    ...
    for (prev = NULL, node = _jitc->head; node; node = next)
    {
        ...
        case jit_code_save:
            ...
            else
            {
                ...
                for (value = 0; value < _jitc->reglen; value++)
                {
                    if (value != regno && ((jit_class(_rvs[value].spec) & spec) & ~jit_class_arg) == spec &&
                        !jit_regset_tstbit(&_jitc->regarg, value) && !spill_reglive_p(node, value))
                        break;
                }
                if (value < _jitc->reglen)
                {
